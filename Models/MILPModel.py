...
class MILPModel(pyo.AbstractModel):
    ...
    def __build_parameters(self):
        ...
        self.L = pyo.RangeSet(1, self.N_l) # Set of locations
    ...
    def __build_constraints(self):
        ...
        self.Cons_balance_portfolio  = pyo.Constraint(self.T, rule=aux.energy_balance_portfolio) # This will be relaxed later!
    ...
    def __build_objective(self):
        self.Objective_Cost = pyo.Objective(rule=aux.cost, sense=pyo.minimize) #Objective_Cost_MILP
    ...
    def build_instance(self, instance_size:int, equal_prices:bool, site_id =-1):
        ...
        dict_data['N_l']          = {None: df_size.iloc[0]['n_locations']}
        ...
        dict_data['LB']       = {None: [(l,b) for (l,b) in df_lb_set[['site','battery']].to_records(index=False) ]}
        ...
        dict_data['initial_soc']    = {(l,b): df_lb[(df_lb['site']==l) & (df_lb['battery']==b)].iloc[0]['initial_soc']\
                                                for (l,b) in df_lb[['site','battery']].to_records(index=False) }
