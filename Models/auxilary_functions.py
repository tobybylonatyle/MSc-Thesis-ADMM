...
def cost_ALR_location(m):
    ...
    homerun = sum((m.dualgamma[t]/2)*((m.commitment_i[t] + m.i_G[t] + sum(m.e_S_prime[t,l] for l in m.L_prime if l != m.L) + sum(m.e_S[t,l] for l in m.L) - m.commitment_e[t] - m.e_G[t] - sum(m.i_S_prime[t,l] for l in m.L_prime if l != m.L) - sum(m.i_S[t,l] for l in m.L))**2) for t in m.T)
